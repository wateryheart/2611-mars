...
#---------------------------------------------------
# Step 1:
# check whether there are avaiable *Simple* bombs to use.
# if yes, current number of bomb - 1
#bomb_ids: 	.word -1:10 # used to keep track of the ids of bombs
#bomb_base:	.word 50 # base id of the simple bomb
#bomb_num:	.word 0 # the number of simple bombs
#bomb_count:	.word 0 # the "running" number of simple bombs
#---------------------------------------------------
la $t0, bomb_num
lw $t1, 0($t0) # t1 = number of available simple bomb
beq $t1, $zero, end # if number of bomb = 0, break the function
# if not, we will create a bomb
la $t6, rbomb_num
lw $t7, 0($t6)

#---------------------------------------------------
# we need to deduct the display available bomb by 1
addi $t1, $t1, -1
li $v0, 123
add $a0, $t1, $zero
add $a1, $t7, $zero
syscall
# we need to deduct current available bomb by 1
sw $t1, 0($t0)
# we need to increase current "running" bomb by 1
la $t9, bomb_count
lw $t8, 0($t9)
addi $t8, $t8, 1
sw $t8, 0($t9)
#---------------------------------------------------
#====================================
# we need id, x_coord, y_coord, speed
la $t0, bomb_count
lw $t2, 0($t0) # t2 = number of running simple bomb
la $t0, bomb_ids
sll $t1, $t1, 2
add $t0, $t0, $t1
lw $t3, 0($t0) # t3 = the corresponding id
# i.e if there are 3 bombs moving in the screen, 
# t3 will be the id of the 4th one
# now we need x,y_coord of the ship
li $v0, 110
li $a0, 1 # id of ship = 1
syscall
add $t4, $v0, $zero # $t4 = x_coord
add $t5, $v1, $zero # $t5 = y_coord
...
#---------------------------------------------------
# Step 1:
# check whether there are avaiable *remote* bombs to use.
# if yes, current number of bomb - 1
#rbomb_ids:	.word -1:10 # used to keep track of the ids of remote bombs
#rbomb_base:	.word 80 # base id of the remote bomb
#rbomb_num:	.word 0 # the number of remote bombs
...
#---------------------------------------------------
la $t0, rbomb_num
lw $t1, 0($t0) # t1 = number of available remote bomb
beq $t1, $zero, end_rbomb # if number of rbomb = 0, break the function
# if not, we will create a rbomb

la $t6, bomb_num
lw $t7, 0($t6) 
#---------------------------------------------------
# we need to deduct the display available rbomb by 1
addi $t1, $t1, -1
li $v0, 123
add $a0, $t7, $zero
add $a1, $t1, $zero
syscall
# we need to deduct current available rbomb by 1
sw $t1, 0($t0)
# we need to increase current "running" rbomb by 1
la $t9, rbomb_count
lw $t8, 0($t9)
addi $t8, $t8, 1
sw $t8, 0($t9)
...
# we need id, x_coord, y_coord, speed
la $t0, rbomb_count
lw $t2, 0($t0) # t2 = number of running rbomb
la $t0, rbomb_ids
sll $t1, $t1, 2
add $t0, $t0, $t1
lw $t3, 0($t0) # t3 = the corresponding id
# i.e if there are 3 rbombs moving in the screen, 
# t3 will be the id of the 4th one
# now we need x,y_coord of the ship
li $v0, 110
li $a0, 1 # id of ship = 1
...
# $t7 = x_coord
#$t6 = speed
#$t8 = current direction
#$t4 = number of dophin
#----------------------------
# create a while loop 
#----------------------------
la $t0, dolphin_num
lw $t4, 0($t0) #$t4 = number of dophin
li $t2, 0 # set up a counter for looping the id
#----------------------------
While_loop:
beq $t4, $zero, end_the_loop
#----------------------------

la $t3, dolphin_ids
sll $t0, $t2, 2 # the number of stack 0 ->  4 -> 8
add $t3, $t3, $t0 # the current address + the number of stack = the destination address for the corresponding id
lw $t5, 0($t3) #$t5 = id 
#-----------------------------
# get dolphin  location
li $v0, 110
add $a0, $t5, $zero
syscall
#-----------------------------
# then $v0 now stores the object x_coord
# then $v1 now stores the object y_coord
# store it in a temp
add $t7, $v0, $zero # $t7 = x_coord
#-----------------------------
# get dolphin speed
li $v0, 111
add $a0, $t5, $zero
syscall
#-----------------------------
# then $v0 now stores the object speed
# store it in a temp
add $t6, $v0, $zero #$t6 = speed
#-----------------------------
...
# $v0 = 1: Right
# $v0 = 0: Left
add $t8, $v0, $zero #$t8 = current direction
bne $t8, $zero, Move_right_check

Move_left_check:
sub $t3, $t6, $t7 # next location = x-coord - speed
li $t1, 0 # hit the boundary
slt $t0, $t3, $t1
bne  $t0, $zero, refresh
li $t1, 1
#------------------------------
...
Move_right_check:
add $t0, $t6, $t7 # next location = x-coord + speed
li $t1, 740 # hit the boundary
# if next location hit the boundary, we need to change direction
slt $t9, $t0, $t1
bne $t9, $zero, refresh
li $t1, 0
#------------------------------
...
#----------------------------
la $t0, subma_num
lw $t4, 0($t0) #$t4 = number of subma
li $t2, 0 # set up a counter for looping the id
#----------------------------
While_loop_subma:
beq $t4, $zero, end_loop_subma
#----------------------------
...
# $v0 = 0: Left
add $t8, $v0, $zero #$t8 = current direction
bne $t8, $zero, Move_right_check_subma

Move_left_check_subma:
sub $t3, $t6, $t7 # next location = x-coord - speed
li $t1, 0 # hit the boundary
slt $t0, $t3, $t1
bne  $t0, $zero, refresh_subma
li $t1, 1
#------------------------------
# change the direction to right
li $v0, 113
...
Move_right_check_subma:
add $t0, $t6, $t7 # next location = x-coord + speed
li $t1, 720 # hit the boundary
# if next location hit the boundary, we need to change direction
slt $t9, $t0, $t1
bne $t9, $zero, refresh_subma
li $t1, 0
#------------------------------
# change the direction to left
li $v0, 113
...
#--------------------------------------------------------------------
la $t0, bomb_num
lw $t4, 0($t0) # t4 = number of available bomb

la $t0 ,bomb_count
lw $t9, 0($t0)

la $t0, rbomb_num
lw $t8, 0($t0) # t8 = number of available rbomb

la $t0, rbomb_count
lw $t7, 0($t0)

while_loop2:
beq $t9, $zero, end_loop2
sll $t5, $t4, 2

la $t0, bomb_ids
add $t0, $t0, $t5
lw $t1, 0($t0) # t1 = the corresponding id
